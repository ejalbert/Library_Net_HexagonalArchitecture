...
{
    [Fact]
    public async Task Update_passes_id_title_and_author_to_port_and_returns_updated_book()
    {
        Mock<IUpdateBookPort> portMock = new();
        Mock<ILogger<UpdateBookService>> loggerMock = new();
        Book updated = new() { Id = "book-123", Title = "Clean Architecture (2nd Ed.)", AuthorId = "author-1" };

        portMock.Setup(port => port.Update("book-123", "Clean Architecture (2nd Ed.)", "author-1"))
        ...
        portMock.Verify(port => port.Update("book-123", "Clean Architecture (2nd Ed.)", "author-1"), Times.Once);
    }
}
