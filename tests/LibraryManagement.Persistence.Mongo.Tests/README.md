...
Covers the MongoDB adapter end-to-end using xUnit and Testcontainers. The suite validates mapper logic and adapter
behaviour for the `Books` aggregate.

## Notable Fixtures

- `MongoDbContainerFixture` spins up MongoDB 7 via Testcontainers, provides isolated databases per test, and drops them
  during teardown.
