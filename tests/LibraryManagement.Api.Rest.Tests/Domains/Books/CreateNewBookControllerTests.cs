...
public class CreateNewBookControllerTests
{
    ...
    public async Task CreateNewBook_ReturnsOkWithBookFromUseCase()
    {
        ...
        var expected = new BookDto() { Id = book.Id, Title = book.Title, AuthorId = book.AuthorId };
        ...
        mapperMock.Setup(x => x.ToDto(It.IsAny<Book>())).Returns(new BookDto { Id = book.Id, Title = book.Title, AuthorId = book.AuthorId });

        var controller = new CreateNewBookController(useCaseMock.Object, mapperMock.Object);
        var request = new CreateNewBookRequestDto("The Hobbit", "author-1");
        ...
        useCaseMock.Verify(
            x => x.Create(It.Is<CreateNewBookCommand>(command => command == new CreateNewBookCommand(request.Title, request.AuthorId))),
            Times.Once);
    }
}
