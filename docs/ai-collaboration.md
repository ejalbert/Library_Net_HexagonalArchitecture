...
This repository is intended to be co-developed by humans and coding assistants such as GitHub Copilot and Codex CLI agents to deliver a modern Library Management System.
...
- **Explicit context** – When creating new modules or patterns, document the intent using docstrings or comments where it aids comprehension, especially for circulation policies, patron notifications, and catalogue rules.
...
- Track open questions in `TODO:` comments or GitHub issues; include enough context for AIs to act on them without guesswork (e.g., specify which loan policy or branch workflow requires clarification).
...
- When creating a new project (under `src/`, `tests/`, or adapters), scaffold its paired `{Project}.Tests` project at the same time and add a `README.md` in each directory describing scope, dependencies, and how to build/test it.
