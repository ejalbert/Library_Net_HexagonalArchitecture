...
public class SearchAuthorsChatTools(ISearchAuthorsUseCase searchAuthorsUseCase) : SearchChatToolBase<Author>(
    nameof(SearchAuthorsChatTools), ChatTool.CreateFunctionTool(
        nameof(SearchAuthorsChatTools),
        """
...
{
    protected override Task<SearchResult<Author>> ExecuteSearchAsync(string? searchTerm, Pagination? pagination,
        JsonDocument arguments)
    {
        return searchAuthorsUseCase.Search(new SearchAuthorsCommand(searchTerm, pagination));
    }
}
