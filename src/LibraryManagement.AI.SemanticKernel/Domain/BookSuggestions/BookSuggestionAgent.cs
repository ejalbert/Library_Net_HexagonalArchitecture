...
public class BookSuggestionAgent([FromKeyedServices(nameof(BookSuggestionAgent))]Kernel kernel, IChatCompletionService chatCompletionService, ILogger<BookSuggestionAgent> logger): IBookSuggestionAgent
{
    public async Task<string> SuggestAsync(string prompt, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var chatContents = await chatCompletionService.GetChatMessageContentsAsync(chatHistory, new PromptExecutionSettings()
            {
                FunctionChoiceBehavior = FunctionChoiceBehavior.Auto()
            }, kernel, cancellationToken);


            return chatContents.Last().Content ?? "No content generated.";
        }
        catch (Exception ex)
        {
