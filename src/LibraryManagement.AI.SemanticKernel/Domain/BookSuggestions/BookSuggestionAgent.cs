...
public class BookSuggestionAgent(
    [FromKeyedServices(nameof(BookSuggestionAgent))] Kernel kernel,
    IChatCompletionService chatCompletionService,
    ILogger<BookSuggestionAgent> logger) : IBookSuggestionAgent
{
    public async Task<string> SuggestAsync(string prompt, CancellationToken cancellationToken)
    {
        try
        {
            ...
            IReadOnlyList<ChatMessageContent> chatContents = await chatCompletionService.GetChatMessageContentsAsync(
