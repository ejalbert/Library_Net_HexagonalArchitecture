...
{
    public async Task<Book> Patch(string id, string? title, string? authorId, string? description, IReadOnlyCollection<string>? keywords)
    {
        ...
        {
            var cursor = await bookCollection.Collection.FindAsync(entity => entity.Id == id);
    ...
    private static UpdateDefinition<BookEntity>? BuildUpdateDefinition(string? title, string? authorId, string? description, IReadOnlyCollection<string>? keywords)
    {
        List<UpdateDefinition<BookEntity>> updates = new();
        if (title is not null)
        {
            updates.Add(Builders<BookEntity>.Update.Set(entity => entity.Title, title));
        }

        if (authorId is not null)
        {
            updates.Add(Builders<BookEntity>.Update.Set(entity => entity.AuthorId, authorId));
        }
