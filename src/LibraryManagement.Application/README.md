...
ASP.NET Core host that composes every module (domain, Mongo and Postgres persistence, REST API, OpenAI adapter, and
Blazor UI).
...
- `ConnectionStrings:postgres` – preferred Postgres connection string (fallback:
...
- `LibraryManagement.Persistence.Postgres`: EF Core adapter + migrations assembly for book/author persistence and AI
  consumption logging.
