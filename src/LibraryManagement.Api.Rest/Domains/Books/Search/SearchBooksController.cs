...
public class SearchBooksController(ISearchBooksUseCase searchBooksUseCase, IBookDtoMapper mapper, ISearchDtoMapper searchDtoMapper)
    : ISearchBooksController
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var searchResult = await searchBooksUseCase.Search(new SearchBooksCommand(requestDto.SearchTerm, pagination));

        return Results.Ok(new SearchBooksResponseDto(searchResult.Results.Select(mapper.ToDto), searchDtoMapper.ToDto(searchResult.Pagination)));
    }
}
