...
public class CreateAuthorController(ICreateAuthorUseCase createAuthorUseCase, IAuthorDtoMapper mapper)
    : ICreateAuthorController
{
    public async Task<IResult> CreateAuthor(CreateAuthorRequestDto request)
    {
        Author author = await createAuthorUseCase.Create(new CreateAuthorCommand(request.Name));
